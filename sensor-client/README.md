...
이 프로그램은 Y16 값이 TLinear 방식의 0.01 K 단위로 전달되는 구성을 전제로 합니다. 기본 화면은 작은 온도 흔들림과 고정 패턴이 과장되지 않도록 시간 필터와 경계 보존 필터를 적용합니다. 온도 계산과 CSV 및 Y16 저장에는 필터를 적용하지 않은 원본을 사용합니다.
