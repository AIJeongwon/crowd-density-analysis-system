...
- 상단 상태는 최근 유효한 센서 데이터가 한 지점 이상 있으면 `LIVE`, 정상 조회했지만
  데이터가 없으면 `대기`, 조회 실패 또는 시간 초과이면 `ERROR`입니다.
  최초 연결 확인도 `대기`로 표시하며, 데모 모드는 실제 데이터와 구분해 `DEMO`를 유지합니다.
  센서 전송이 끊기면 마지막 데이터가 유효 시간(기본 30초)을 지난 뒤 다음 조회에서 `대기`로 바뀝니다.
  인원수가 0명이어도 정상 수신한 데이터라면 `LIVE`입니다. 실제 장치 연결 여부를 직접 확인하는 표시는 아닙니다.
